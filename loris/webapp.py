...
def get_debug_config(debug_jp2_transformer):
    ...
    if debug_jp2_transformer == "opj":
        ...
        raise ConfigError(
            "Unrecognized debug JP2 transformer: %r" % debug_jp2_transformer
        )
...
def read_config(config_file_path):
    ...
    config["DEFAULT"] = {
        key: val for (key, val) in os.environ.items() if key not in ("PS1")
    }
    return config
...
class LorisResponse(BaseResponse, CommonResponseDescriptorsMixin):
    ...
    def __init__(self, response=None, status=None, content_type=None):
        super(LorisResponse, self).__init__(
            response=response, status=status, content_type=content_type
        )
...
class Loris:
    def __init__(self, app_configs={}):
        ...
        [
            ...
            for sub_key in self.app_configs[key]
        ]
        ...
        self.redirect_canonical_image_request = _loris_config[
        ...
        if self.enable_caching:
            self.info_cache = InfoCache(
                self.app_configs["img_info.InfoCache"]["cache_dp"]
            )
    ...
    def _load_transformers(self):
        ...
        global_transform_options = dict(
            (k, v) for k, v in tforms.items() if not isinstance(v, dict)
        )
        self.logger.debug("Global transform options: %r", global_transform_options)

        pil_max_image_pixels = tforms.get(
            "pil_max_image_pixels", Image.MAX_IMAGE_PIXELS
        )
        if pil_max_image_pixels != 0:
            Image.MAX_IMAGE_PIXELS = pil_max_image_pixels
            self.logger.debug(
                "PIL maximum image pixels set to: %d", pil_max_image_pixels
            )
        else:
        ...
        for sf in source_formats:
            ...
            config = dict(
                list(self.app_configs["transforms"][sf].items())
                + list(global_transform_options.items())
            )
    ...
    def route(self, request):
        loris_request = LorisRequest(
            request, self.redirect_id_slash_to_info, self.proxy_path
        )
        ...
        if request_type == "redirect_info":
            ...
            return self.get_img(
                request, ident, region, size, rotation, quality, fmt, base_uri
            )
    ...
    def _set_canonical_link(self, request, response, image_request, image_info):
        if self.proxy_path:
    ...
    def get_img(
        self, request, ident, region, size, rotation, quality, target_fmt, base_uri
    ):
        """Get an Image.
        Args:
            request (Request):
        ...
        image_request = img.ImageRequest(
            ident, region, size, rotation, quality, target_fmt
        )
        ...
        if in_cache:
            ...
            try:
                ...
                if image_request.request_resolution_too_large(
                    max_size_above_full=self.max_size_above_full, image_info=info
                ):
                ...
                # Make an image
                fp = self._make_image(image_request=image_request, image_info=info)

            except TransformException as te:
                return ServerSideErrorResponse(
                    "error generating derivative image: see log"
                )
...
(%s).""" % (
        ...
        r.headers["Content-Length"] = path.getsize(fp)
        self._set_canonical_link(
            request=request, response=r, image_request=image_request, image_info=info
        )
        r.response = open(fp, "rb")
    ...
    def _make_image(self, image_request, image_info):
        ...
        temp_file = NamedTemporaryFile(
            dir=self.tmp_dp, suffix=".%s" % image_request.format, delete=False
        )
        ...
        try:
            ...
            transformer.transform(
                target_fp=temp_fp, image_request=image_request, image_info=image_info
            )
            derivative_size = os.stat(temp_fp).st_size
            if derivative_size < 1:
                self.logger.error(
                    "empty derivative file created for %s" % image_info.src_img_fp
                )
        ...
        if self.enable_caching:
            canonical_cache_fp = self.img_cache.upsert(
                image_request=image_request, temp_fp=temp_fp, image_info=image_info
            )
...
if __name__ == "__main__":
    ...
    app = create_app(config_file_path="/opt/loris/etc/loris.conf")  # or 'opj'
